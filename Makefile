...
all: $(VOFILES) $(CMOFILES) $(if $(HASNATDYNLINK_OR_EMPTY),$(CMXSFILES)) test\
  ...
  rauzy/algorithme1/extraction/suresnes.ml
...
uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/CanonBDDs && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "CanonBDDs" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
...
clean::
	...
	- rm -rf rauzy/algorithme1/extraction/suresnes.ml
